...
export class TaskRepository extends BaseRepository<ITask> {
  ...
  async findTasks(
    ...
    const query: FilterQuery<ITask> = { createdBy: new Types.ObjectId(userId) };
    ...
    if (filters.templateId !== undefined) {
      query.templateId = filters.templateId ? new Types.ObjectId(filters.templateId) : null;
    }
   ...
   */
  async findRecurringTemplates(userId: string): Promise<ITask[]> {
    return this.model
      .find({
   ...
   */
  async findInstanceByTemplateAndDate(
    templateId: string,
    startOfDay: Date,
    endOfDay: Date
  ): Promise<ITask | null> {
