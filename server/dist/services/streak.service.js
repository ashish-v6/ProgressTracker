...
class StreakService {
    ...
    async recalculateStreak(userId) {
        ...
        const completedTasks = await task_repository_1.taskRepository.find({ createdBy: userId, completed: true, completedAt: { $ne: null } }, 'completedAt', { sort: { completedAt: 1 } });
        if (completedTasks.length === 0) {
            await user_repository_1.userRepository.update(userId, { streak: 0, lastActiveDate: null });
            return { currentStreak: 0, longestStreak: 0 };
        }
        // Format completedAt values to YYYY-MM-DD strings based on Local Time
        // to map user experience accurately.
        const uniqueLocalDates = Array.from(new Set(completedTasks.map((task) => {
...
exports.default = exports.streakService;
