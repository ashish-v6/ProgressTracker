...
class AnalyticsService {
    ...
    async getDashboardData(userId) {
        ...
        const todayTasks = await task_service_1.taskService.resolveRecurringTasksForDate(userId, today);
        ...
        const weeklyTasks = await task_service_1.taskService.resolveTasksForDateRange(userId, sevenDaysAgo, today);
        ...
        const monthlyTasks = await task_service_1.taskService.resolveTasksForDateRange(userId, thirtyDaysAgo, today);
        ...
        const allTasks = await task_service_1.taskService.queryTasks(userId, {}, { limit: 100000 });
        let totalHours = 0;
    ...
    async getAnalyticsData(userId) {
        ...
        for (let i = 3; i >= 0; i--) {
            ...
            const tasks = await task_service_1.taskService.resolveTasksForDateRange(userId, startOfWeek, endOfWeek);
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const tasks = await task_service_1.taskService.resolveRecurringTasksForDate(userId, start);
            const total = tasks.length;
            const completed = tasks.filter(t => t.completed).length;
            let hours = 0;
            tasks.forEach(t => hours += this.getActualHoursDecimal(t));
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const tasks = await task_service_1.taskService.resolveTasksForDateRange(userId, startOfMonth, endOfMonth);
            const total = tasks.length;
            const completed = tasks.filter(t => t.completed).length;
            let hours = 0;
            tasks.forEach(t => hours += this.getActualHoursDecimal(t));
        ...
        const yearlyTasks = await task_service_1.taskService.resolveTasksForDateRange(userId, startOfYear, endOfYear);
        ...
        const allTasksRes = await task_service_1.taskService.queryTasks(userId, {}, { limit: 100000 });
        ...
        const last30DaysTasks = await task_service_1.taskService.resolveTasksForDateRange(userId, new Date(today.getTime() - (29 * 24 * 60 * 60 * 1000)), today);
        ...
        const yearTasks = await task_service_1.taskService.resolveTasksForDateRange(userId, oneYearAgo, today);
        const heatmapMap = {};
        yearTasks.forEach(t => {
    ...
    async getCalendarData(userId, year, month) {
        ...
        for (let m = 1; m <= 12; m++) {
            ...
            const monthTasks = await task_service_1.taskService.resolveTasksForDateRange(userId, start, end);
