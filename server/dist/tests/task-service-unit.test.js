...
describe('TaskService Unit Tests', () => {
    ...
    it('should cover repeat rules in createTask', async () => {
        const rules = ['weekdays', 'weekends', 'daily', 'weekly'];
        for (const rule of rules) {
            const task = await task_service_1.taskService.createTask(userId.toString(), {
                title: `Task for ${rule}`,
                category: 'Test',
                color: '#4F46E5',
                repeatRule: rule,
    ...
    it('should throw NotFoundError in getTaskById if task does not exist', async () => {
        ...
        await expect(task_service_1.taskService.getTaskById(userId.toString(), fakeId)).rejects.toThrow('Task not found');
    });
    it('should throw NotFoundError in getTaskById if task belongs to another user', async () => {
        ...
        await expect(task_service_1.taskService.getTaskById(userId.toString(), task.id)).rejects.toThrow('Task not found');
    });
    it('should cover resolveRecurringTasksForDate with weekday and weekly templates', async () => {
        ...
        const weekdaysTemplate = await task_model_1.Task.create({
        ...
        const weekendsTemplate = await task_model_1.Task.create({
        ...
        await task_service_1.taskService.resolveRecurringTasksForDate(userId.toString(), monday);
        const mondayTasks = await task_model_1.Task.find({ templateId: weekdaysTemplate._id });
        expect(mondayTasks.length).toBe(1);
        // Test resolving on a Sunday (day 0)
        const sunday = new Date('2026-07-19T10:00:00.000Z'); // Sunday
        await task_service_1.taskService.resolveRecurringTasksForDate(userId.toString(), sunday);
        const sundayTasks = await task_model_1.Task.find({ templateId: weekendsTemplate._id });
        expect(sundayTasks.length).toBe(1);
    });
    it('should cover updateTask and throw error if not found', async () => {
        const fakeId = new mongoose_1.default.Types.ObjectId().toString();
        await expect(task_service_1.taskService.updateTask(userId.toString(), fakeId, { title: 'Updated' })).rejects.toThrow('Task not found');
    });
    it('should cover deleteTask and throw error if not found', async () => {
        const fakeId = new mongoose_1.default.Types.ObjectId().toString();
        await expect(task_service_1.taskService.deleteTask(userId.toString(), fakeId)).rejects.toThrow('Task not found');
    });
    it('should cover recurring task service getRecurringTaskById errors', async () => {
        const fakeId = new mongoose_1.default.Types.ObjectId().toString();
        await expect(recurring_task_service_1.recurringTaskService.getRecurringTaskById(userId.toString(), fakeId)).rejects.toThrow('Recurring task template not found');
        const template = await recurring_task_model_1.RecurringTask.create({
        ...
        await expect(recurring_task_service_1.recurringTaskService.getRecurringTaskById(userId.toString(), template.id)).rejects.toThrow('Recurring task template not found');
    });
    it('should cover updateRecurringTask and throw error if not found', async () => {
        const fakeId = new mongoose_1.default.Types.ObjectId().toString();
        await expect(recurring_task_service_1.recurringTaskService.updateRecurringTask(userId.toString(), fakeId, { title: 'Updated' })).rejects.toThrow('Recurring task template not found');
    });
    it('should cover generating tasks for custom and monthly recurring tasks', async () => {
        const customTemplate = await recurring_task_model_1.RecurringTask.create({
        ...
        const monthlyTemplate = await recurring_task_model_1.RecurringTask.create({
        ...
        await recurring_task_service_1.recurringTaskService.generateTasksForDateRange(userId.toString(), startDate, endDate);
        const customTasks = await task_model_1.Task.find({ templateId: customTemplate._id });
        expect(customTasks.length).toBe(2);
        const monthlyTasks = await task_model_1.Task.find({ templateId: monthlyTemplate._id });
        expect(monthlyTasks.length).toBe(1);
    });
    it('should skip task generation if target date is before template creation date', async () => {
        const futureTemplate = await recurring_task_model_1.RecurringTask.create({
        ...
        await recurring_task_service_1.recurringTaskService.generateTasksForDateRange(userId.toString(), new Date('2026-07-20T00:00:00.000Z'), new Date('2026-07-20T23:59:59.000Z'));
        const tasks = await task_model_1.Task.find({ templateId: futureTemplate._id });
        expect(tasks.length).toBe(0);
    });
    it('should skip task service resolve if template created after end of target date', async () => {
        const futureTemplate = await task_model_1.Task.create({
        ...
        await task_service_1.taskService.resolveRecurringTasksForDate(userId.toString(), new Date('2026-07-20T12:00:00.000Z'));
        const tasks = await task_model_1.Task.find({ templateId: futureTemplate._id });
        expect(tasks.length).toBe(0);
    });
    it('should cover doesTemplateMatchDate match conditions', () => {
        const fn = recurring_task_service_1.recurringTaskService.doesTemplateMatchDate.bind(recurring_task_service_1.recurringTaskService);
        const base = {
            createdAt: new Date('2026-07-01'),
            targetHours: 1,
            targetMinutes: 0
        ...
        expect(fn({ ...base, repeatRule: 'invalid' }, new Date('2026-07-15'))).toBe(false);
    });
    it('should cover taskService.doesTemplateMatchDate match conditions', () => {
        const fn = task_service_1.taskService.doesTemplateMatchDate.bind(task_service_1.taskService);
        const base = {
    ...
    it('should resolve tasks for date range with active recurring and legacy templates of all repeat rules', async () => {
        const originalList = recurring_task_service_1.recurringTaskService.listRecurringTasks.bind(recurring_task_service_1.recurringTaskService);
        jest.spyOn(recurring_task_service_1.recurringTaskService, 'listRecurringTasks').mockImplementationOnce(async (uid) => {
            ...
            return [
                ...templates,
                {
                    ...
                    repeatRule: 'invalid',
                    repeatDays: [],
                    createdBy: new mongoose_1.default.Types.ObjectId(uid),
                    targetHours: 1,
                    targetMinutes: 0,
        ...
        const tasks = await task_service_1.taskService.resolveTasksForDateRange(userId.toString(), start, end);
        expect(tasks.length).toBeGreaterThan(0);
    });
});
//# sourceMappingURL=task-service-unit.test.js.map